...
After a managed session registers successfully, the extension adds an active `campfire_update` tool. Its prompt guidance asks Pi to report meaningful plans, discoveries, setbacks, recoveries, test results, milestones, and handoffs using a factual, conversational message of at most 140 characters. Routine operations, ordinary next steps, and repeated information are explicitly excluded. `milestone` is reserved for tangible completed results, while `handoff` requires an actual transfer of responsibility or context; neither should be used merely to bypass pacing. Campfire events use the existing agent-event hook and do not change agent status. The hook rejects unknown kinds, control characters, and oversized messages; deduplicates consecutive messages; rate-limits routine updates to one per 90 seconds; and retains the latest 25 messages per session. `snag`, `milestone`, and `handoff` events bypass the time limit.
...
In kanban layouts at least 120 columns wide, a persistent `Campfire` rail appears to the right of the board. It merges the bounded activity lists from all visible sessions, sorts them newest-first, and shows each message's kind, project, relative age, and text. Narrower layouts keep the full-width board and omit the rail.
