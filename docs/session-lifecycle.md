...
After a managed session registers successfully, the extension adds an active `campfire_update` tool. Its prompt guidance asks Pi to report meaningful plans, discoveries, setbacks, recoveries, test results, milestones, and handoffs using a factual, conversational message of at most 140 characters. Routine operations and repeated information are explicitly excluded. Campfire events use the existing agent-event hook and do not change agent status. The hook rejects unknown kinds, control characters, and oversized messages; deduplicates consecutive messages; rate-limits routine updates to one per 90 seconds; and retains the latest 25 messages per session. `snag`, `milestone`, and `handoff` events bypass the time limit.
