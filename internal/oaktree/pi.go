...
export default function (pi) {
  ...
  function registerCampfireTool() {
    ...
    pi.registerTool({
      ...
      label: "Campfire",
      description: "Post one short, lively, truthful update to the shared oak-tree Campfire feed.",
      promptSnippet: "Post lively, useful shared-feed updates to the oak-tree Campfire",
      promptGuidelines: [
        ...
        "Write like a teammate posting to a shared Campfire: brisk, conversational, and with a little personality—not like a tool log.",
        "Lead with the concrete development: ‘Found the culprit: the cache key drops the locale. Fixing it now.’; ‘Tests are green—47 passed. Taking the migration for one last lap.’; ‘Hit a snag: the fixture lies about expiry. Switching to a real token.’",
        "Use handoff for delegation and name the task and destination when safe: ‘Handing auth cleanup to api-session; I’ll wire up the UI.’",
        "Skip only routine reads, searches, commands, tiny tasks, and information already posted recently.",
        "Keep messages truthful, specific, and under 140 characters; include the next action when useful, but never invent progress or manufacture drama.",
