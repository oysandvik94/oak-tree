...
func TestEnsurePiExtensionContainsLifecycleAndQuestionTool(t *testing.T) {
	...
	for _, want := range []string{"session_start", "agent_settled", "session_shutdown", "registerTool", "registerCommand(\"oak-tree\"", "Usage: /oak-tree register", "managed: ctx.mode === \"tui\"", "!extra.managed", "tmux_pane", "getAllTools", "question", "promptGuidelines: [", "promptSnippet:", "executionMode: \"sequential\"", "rpiv:ask-user:prompt", "ask_user_question", "tool_execution_end", "result.code === 0", "todoSummary", "message.toolName === \"todo\"", "event.toolName === \"todo\"", "todo_in_progress", "todo_json", "task.subject.trim()", "campfire_update", "campfire_read", "campfire-read", "activity_kind", "activity_message", "StringEnum", "meaningful plan", "starting a distinct phase", "delegating work", "notable delegated result", "prefer a useful update over silence", "shared room, not a status API", "write in first person", "genuine difficulty", "This is hard", "I’m proud of this one", "Back to the trace", "do not flatten every post", "name the task and destination", "Handing auth cleanup", "Skip campfire_update only", "never invent feelings", "ambient awareness", "not a coordination channel", "Do not poll Campfire"} {
