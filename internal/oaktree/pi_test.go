...
func TestEnsurePiExtensionContainsLifecycleAndQuestionTool(t *testing.T) {
	...
	for _, want := range []string{"session_start", "agent_settled", "session_shutdown", "registerTool", "registerCommand(\"oak-tree\"", "Usage: /oak-tree register", "managed: ctx.mode === \"tui\"", "!extra.managed", "tmux_pane", "getAllTools", "question", "promptGuidelines: [", "promptSnippet:", "executionMode: \"sequential\"", "rpiv:ask-user:prompt", "ask_user_question", "tool_execution_end", "result.code === 0", "todoSummary", "message.toolName === \"todo\"", "event.toolName === \"todo\"", "todo_in_progress", "todo_json", "task.subject.trim()", "campfire_update", "activity_kind", "activity_message", "StringEnum", "meaningful plan", "starting a distinct phase", "delegating work", "notable delegated result", "prefer a useful update over silence", "teammate posting to a shared Campfire", "not like a tool log", "Found the culprit", "Tests are green", "Hit a snag", "name the task and destination", "Handing auth cleanup", "Skip only routine reads", "never invent progress", "manufacture drama"} {
