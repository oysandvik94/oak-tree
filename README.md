...
Wide kanban layouts include a live `Campfire` rail beside the board. Managed Pi sessions receive a `campfire_update` tool with instructions to post short, lively, truthful shared-feed updates after meaningful plans, discoveries, setbacks, recoveries, test results, phase changes, milestones, and handoffs. The voice is a brisk teammate update—not a tool log—while remaining specific and honest. Delegating work to another agent or session—and receiving a notable delegated result—should produce an update naming the task and destination when safe. Only routine tool calls, tiny tasks, and recently repeated information stay silent. The dashboard combines updates from all visible sessions newest-first, with message kinds rendered as colored labels.
